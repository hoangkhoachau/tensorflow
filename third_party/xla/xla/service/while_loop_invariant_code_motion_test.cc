...
TEST_F(WhileLoopInvariantCodeMotionTest, HoistsGetRngSeed) {
  auto m = CreateNewVerifiedModule();
  auto array_u32 = ShapeUtil::MakeShape(U32, {1});
  Shape while_shape = ShapeUtil::MakeTupleShape({array_u32, array_u32});
  ...
  HloComputation* while_body = [&]() {
    ...
    HloInstruction* param = builder.AddInstruction(
        HloInstruction::CreateParameter(0, while_shape, "param"));
    HloInstruction* gte_0 = builder.AddInstruction(
        HloInstruction::CreateGetTupleElement(array_u32, param, 0));
    HloInstruction* rng_seed =
        builder.AddInstruction(HloInstruction::CreateCustomCall(
            ShapeUtil::MakeShape(U32, {}), {}, "GetRngSeed"));
    HloInstruction* bcast = builder.AddInstruction(
        HloInstruction::CreateBroadcast(array_u32, rng_seed, {}));
    HloInstruction* add = builder.AddInstruction(
        HloInstruction::CreateBinary(array_u32, HloOpcode::kAdd, gte_0, bcast));
